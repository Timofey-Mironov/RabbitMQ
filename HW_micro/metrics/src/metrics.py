...
# Словарь -> В нём будем сохранять пропущенные значенмя
...
def write_to_csv(message_id, y_true, y_pred):
    # Формула для расчета абсолютной ошибки
    ...
    with open(csv_file, "a", newline="") as csvfile:
        ...
        # задаём условие на заголовок:
        if not file_exists: # Если файл новый
...
def callback(ch, method, properties, body):
    ...
    # Сохраняем значение в нужный ключ словаря
    ...
    if "true" in pending_values[message_id] and "pred" in pending_values[message_id]:
        ...
        # Записываем метрики в CSV формат
...
# Функция для работы с очередями
def main():
    while True:
        try:
        ...
        # Прописываем ограничениея работы
...
# Скрипт для запуска программы из командной строки
